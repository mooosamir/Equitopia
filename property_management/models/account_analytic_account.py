...
class AccountAnalyticAccount(models.Model):
    ...
    def onchange_property_id(self):
        ...
        if self.property_id:
            ...
            self.rent_type_id = self.property_id.rent_type_id.id or False
    ...
    @api.depends('invc_id', 'invc_id.invoice_payment_state')
    def compute_amount_return(self):
        ...
        for data in self:
            if data.invc_id.invoice_payment_state == 'paid':
    ...
    def button_receive(self):
        ...
        account_jrnl_obj = self.env['account.journal'].search(
            [('type', 'in', ['cash','bank']),('company_id','=', self.company_id.id)], limit=1)
        payment_obj = self.env['account.payment']
        payment_method_id = self.env.ref(
            'account.account_payment_method_manual_in')
        context.update({'close_after_process': True,
                    'account_deposit_received': True})
        for tenancy_rec in self:
    ...
    def button_return(self):
        ...
        for rec in self:
            ...
            inv_values = {
                # 'source': _('Deposit Return For ') + rec.name or "",
                'type': 'in_invoice',
