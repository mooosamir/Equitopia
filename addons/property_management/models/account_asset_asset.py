...
class AccountAssetAsset(models.Model):
    ...
    @api.onchange('gfa_meter')
    def meter_to_sqft(self):
        """
        when you change GFA Feet, this method will change
        GFA Meter field value accordingly.
        @param self: The object pointer
        ...
        feet_val = 0.0
        if self.gfa_meter:
            feet_val = float(self.gfa_meter * 10.7639104)
            self.gfa_feet = feet_val
    ...
    @api.onchange('unit_price', 'gfa_meter')
    def unit_price_calc_meter(self):
        """
        ...
        @param self: The object pointer
        """
        if self.unit_price and self.gfa_meter:
            self.total_price = float(self.unit_price * self.gfa_meter)
            self.value = float(self.unit_price * self.gfa_meter)
        if self.unit_price and not self.gfa_meter:
            raise ValidationError(_('Please Insert GFA(Square meter).'))
    ...
    def unlink(self):
        ...
        for rec in self:
            if rec.state not in ['draft', 'cancel']:
                raise ValidationError(_('You cannot delete property which contract is in Progress or Booked.'))
    ...
    def button_sold(self):
        ...
        for data in self:
            ...
            inv_values = {
                ...
                'payment_id': data.payment_term.id,
